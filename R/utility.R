...
fastmlst_scheme <- function(text){
  ...
  text <- str_replace_all(text, c("\\." = "","candidatus " = "","/" = "","\\(" = "","\\)" = ""))
  
  if(length(unlist(str_split(text, " "))) == 1){
    #"Genus" scheme
    if(grepl("#", text)){
      # Multiple schemes
      split_num <- unlist(str_split(text, "#"))
      genus <- split_num[1]
    ...
    if(grepl("#", text)){
      ...
      species <- unlist(str_split(split[2], "#"))[1]
      scheme_number <- split_num[length(split_num)]
    ...
    if(grepl("#", text)){
      ...
      extra <- paste0(split[2:length(split) - 1],
                      unlist(str_split(split[length(split)], "#"))[1], collapse = "_")
      scheme_number <- split_num[length(split_num)]
...
parse_mlst_profiles <- function(.calls, .profiles){
  ...
  call_names <- .calls %>% select(-Genome, -Scheme, -ST, -any_of(c("clonal_complex"))) %>% names()
  profile_names <- .profiles %>% select(-any_of(c("ST", "clonal_complex", "species"))) %>% names()
  
  if(!setequal(call_names, profile_names))
    warning("Allele names do not match. Ensure correct scheme is being used.")
  
  .complete <- .calls %>%
    select(Genome, all_of(profile_names)) %>%
    separate_rows_sequential(-Genome, sep = "\\|") %>%
    distinct() %>%
    left_join(mutate(.profiles, match = TRUE), by = profile_names) %>%
    unite(col = combined, all_of(profile_names), sep = ",", na.rm = TRUE) %>%
    mutate(error = case_when(
      match ~ NA_character_,
      grepl("~", combined) ~ "new_allele",
      str_count(combined, ",") < length(profile_names) - 1 ~ "missing_allele",
      TRUE ~ "new_st"
    )) %>% 
    group_by(Genome) %>%
    mutate(ST = na_if(paste0(str_sort(na.omit(unique(ST)), numeric = TRUE), collapse = ";"), "")) %>%
    mutate(errors = na_if(paste0(na.omit(unique(error)), collapse = ";"), "")) %>%
    mutate(row = row_number()) %>%
    ungroup() %>%
    select(Genome, ST, errors) %>%
    mutate(assembly = as.character(str_match(Genome,"GCA_\\d+\\.\\d+"))) %>%
    distinct()
...
reparse_mlst <- function(path, save = TRUE){
  raw_mlst <- read.csv(path)
  mlst <- process_mlst(raw_mlst)
  
  pubmlst <- pubmlst_listing() %>%
    mutate(Scheme = map_chr(species, fastmlst_scheme))
  
  current_schema <- raw_mlst %>%
    left_join(pubmlst, by = "Scheme") %>%
    distinct(Scheme, url) %>%
    drop_na()
  
  Scheme <- current_schema %>% pull(Scheme)
  url <- current_schema %>% pull(url)
  
  profile_path <- paste0(dirname(path), "/", str_replace_all(Scheme, "#", ""),".tsv")
  ...
  fixed <- raw_mlst %>%
    ...
    mutate(ST = case_when(is.na(ST) ~ NA_character_, TRUE ~ paste0("ST", ST)))
...
process_mlst <- function(mlst){
  .complete <- mlst %>%
    mutate(mlst_note = case_when(ST == "-" ~ "Missing Allele(s)",
                                 ST == "new_ST" ~ "New ST",
                                 ST == "new_alleles" ~ "New Allele(s)",
                                 TRUE ~ NA_character_)) %>%
    mutate(mlst = recode(ST, "new_ST" = NA_character_,
                         "new_alleles" = NA_character_,
                         "-" = NA_character_)) %>% 
    mutate(assembly = as.character(str_match(Genome,"GCA_\\d+\\.\\d+"))) %>%
    select(assembly, mlst, mlst_note)
...
download_protein_sequences <- function(id, n = 100){
  .data <- tibble(protein = character(), protein_sequence = character())
  ...
  for(start in seq(1, length(id), n)){
    ...
    .data <- add_row(.data, protein = accn, protein_sequence = seq)
...
download_reference_gene_catalog_proteins <- function(path = refgene_path,
  ...
  write_tsv(.sequences, file = paste0(dirname(path),"/",file))
