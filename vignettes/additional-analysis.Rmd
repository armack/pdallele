...
pa %>%
  ...
  dplyr::summarize(count = n_distinct(biosample), adc_st_count = n_distinct(mlst), adc_st_list = paste(unique(mlst), collapse = ", ")) %>%
  arrange(desc(count)) %>%
  readr::write_csv(file.path(manual_output_path, "PA_mlst_by_adc.csv"))
...
pa %>%
  ...
  dplyr::summarize(count = n_distinct(biosample), adc_pds_count = n_distinct(pds), adc_pds_list = paste(unique(pds), collapse = ", ")) %>%
  arrange(desc(count)) %>%
  readr::write_csv(file.path(manual_output_path, "PA_pds_by_adc.csv"))
...
allele_by_pdc <- pa %>%
  ...
  arrange(allele) %>%
  tidyr::drop_na(allele) %>%
  dplyr::summarize(pdc_isolate_count = n_distinct(biosample), adc_allele_count = n_distinct(allele), adc_alleles = paste(unique(allele), collapse = ", "))
...
allele_by_pdc %>%
  full_join(allele_by_oxa, by = join_by(mlst)) %>%
  arrange(desc((pdc_isolate_count + oxa_isolate_count) / 2)) %>%
  readr::write_csv(file.path(manual_output_path, "PA_alleles_by_mlst.csv"))
