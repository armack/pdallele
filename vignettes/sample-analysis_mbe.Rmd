...
for (gene_name in genes_of_interest) {
  ...
  gene_filter_allele_counts_pds <- mbe %>%
    filter_assigned_bla() %>%
  ...
  gene_filter_allele_counts_pds %>%
    readr::write_csv(file.path(
      table_path, paste0(gene_name, "_intrinsic_allele_counts_by_pds.csv")
    ))
  
  # <gene_name> Allele Counts by MLST
  gene_filter_allele_counts_mlst <- mbe %>%
    filter_assigned_bla() %>%
    {if(gene_name == "blaOXA") dplyr::filter(., oxa_family %in% intrinsic_oxa_families)
      else filter_attribute(., allele, gene_name)} %>%
    group_by(mlst) %>%
  ...
  gene_filter_allele_counts_mlst %>%
    readr::write_csv(file.path(
      table_path, paste0(gene_name, "_intrinsic_allele_counts_by_mlst.csv")
    ))

    # <gene_name> MLST Counts by Allele
  gene_filter_mlst_counts_allele <- mbe %>%
    filter_assigned_bla() %>%
    {if(gene_name == "blaOXA") dplyr::filter(., oxa_family %in% intrinsic_oxa_families)
      else filter_attribute(., allele, gene_name)} %>%
  ...
  gene_filter_mlst_counts_allele %>%
    readr::write_csv(file.path(
      table_path, paste0(gene_name, "_mlst_counts_by_intrinsic_allele.csv")
    ))  
  
      # <gene_name> PDS Counts by Allele
  gene_filter_pds_counts_allele <- mbe %>%
    filter_assigned_bla() %>%
    {if(gene_name == "blaOXA") dplyr::filter(., oxa_family %in% intrinsic_oxa_families)
      else filter_attribute(., allele, gene_name)} %>%
    {if(gene_name == "blaOXA") group_by(., allele, oxa_family) 
      else group_by(., allele)} %>%
    summarize(clusters = n_distinct(pds)) %>%
    arrange(desc(clusters))
  
  gene_filter_pds_counts_allele %>%
    readr::write_csv(file.path(
