...
for (gene_name in genes_of_interest) {
  ...
  # <gene_name> Allele Lists and Counts by PDS Cluster
  ...
  gene_filter_alleles_by_pds %>%
    arrange(allele) %>%
    dplyr::summarize(isolates = n_distinct(biosample), alleles = n_distinct(allele), allele_list = paste(unique(allele), collapse = ", ")) %>%
  ...
  gene_filter_alleles_by_mlst %>%
    arrange(allele) %>%
    dplyr::summarize(isolates = n_distinct(biosample), alleles = n_distinct(allele), alleles_list = paste(unique(allele), collapse = ", ")) %>%
    arrange(desc(alleles)) %>%
    readr::write_csv(file.path(
  ...
  gene_filter_mlst_by_allele %>%
    arrange(mlst) %>%
    dplyr::summarize(isolates = n_distinct(biosample), sts = n_distinct(mlst), alleles = paste(unique(mlst), collapse = ", ")) %>%
    arrange(desc(sts)) %>%
    readr::write_csv(file.path(
  ...
  gene_filter_pds_by_allele %>%
    arrange(pds) %>%
    dplyr::summarize(count = n_distinct(biosample), clusters = n_distinct(pds), alleles = paste(unique(pds), collapse = ", ")) %>%
