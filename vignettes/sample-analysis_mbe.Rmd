...
genes_of_interest        <- c("blaPDC","blaOXA")
exclude_genes            <- c("blaI$", "blaR1$")
min_gene_include         <- 5L
allele_label_base        <- "Allele"
family_label_base        <- "Family"
intrinsic_genes          <- c("blaPDC")
intrinsic_oxa_families   <- c("blaOXA-50")
...
```{r, eval = FALSE}
mbe_allele_filtered <- mbe_raw %>%
  filter_microbigge(filter = filter_alleles,
                          deduplicate = TRUE)
...
mbe <- mbe_raw %>%
  filter_microbigge(species = species_of_interest,
                    ...
                    min_gene_count = min_gene_include,
                    exclude = exclude_genes,
                    deduplicate = TRUE) %>%
  filter_required_alleles(genes = genes_of_interest, family = intrinsic_oxa_families) %>%
...
## Isolates Browser data set analysis converted to MBE
### Overview
#### Isolates and genes before and after filtering
```{r, eval = FALSE}
# Unfiltered Isolate and Allele Counts
overview_raw_isolate_count <- mbe_raw %>%
  dplyr::distinct(biosample) %>%
  nrow()

overview_raw_species <- mbe_raw %>%
  dplyr::distinct(biosample, .keep_all = TRUE) %>%
  dplyr::count(species) %>%
  arrange(species) %>%
  {paste0(.$species, " (", .$n, ")", collapse = ", ")}

overview_raw_allele_count <- mbe_raw %>%
  nrow()

overview_raw_distinct_named_allele_count <- mbe_raw %>%
  filter_assigned_bla() %>%
  dplyr::distinct(allele, .keep_all = TRUE) %>%
  nrow()

overview_raw_distinct_genes <- mbe_raw %>%
  dplyr::distinct(gene) %>%
  dplyr::arrange(gene) %>%
  dplyr::pull() %>%
  paste(collapse = ", ")

overview_removed_alleles <- mbe_raw %>%
  dplyr::filter(grepl(paste(remove_mbe_allele_types, collapse = "|"), method)) %>%
  dplyr::count(method)
...
overview_allele_filtered_isolate_count <- mbe_allele_filtered %>%
  dplyr::distinct(biosample) %>%
  nrow()

overview_allele_filtered_species <- mbe_allele_filtered %>%
  dplyr::distinct(biosample, .keep_all = TRUE) %>%
  dplyr::count(species) %>%
  arrange(species) %>%
  {paste0(.$species, " (", .$n, ")", collapse = ", ")}

overview_allele_filtered_allele_count <- mbe_allele_filtered %>%
  nrow()

overview_allele_filtered_distinct_assigned_allele_count <-
  mbe_allele_filtered %>%
  filter_assigned_bla() %>%
  dplyr::distinct(allele, .keep_all = TRUE) %>%
  nrow()

overview_allele_filtered_distinct_genes <- mbe_allele_filtered %>%
  dplyr::distinct(gene) %>%
  dplyr::arrange(gene) %>%
  dplyr::pull() %>%
  paste(collapse = ", ")

overview_removed_alleles <- mbe_allele_filtered %>%
  dplyr::filter(grepl(paste(remove_mbe_allele_types, collapse = "|"), method)) %>%
  dplyr::count(method)
...
overview_isolate_count <- mbe %>%
  dplyr::distinct(biosample) %>%
  nrow()

overview_species <- mbe %>%
  dplyr::distinct(biosample, .keep_all = TRUE) %>%
  dplyr::count(species) %>%
  arrange(species) %>%
  {paste0(.$species, " (", .$n, ")", collapse = ", ")}

overview_allele_count <- mbe %>%
  nrow()

overview_distinct_assigned_allele_count <- mbe %>%
  filter_assigned_bla() %>%
  dplyr::distinct(allele, .keep_all = TRUE) %>%
  nrow()

overview_distinct_genes <- mbe %>%
  dplyr::distinct(gene) %>%
  dplyr::arrange(gene) %>%
  dplyr::pull() %>%
  paste(collapse = ", ")
...
isolates_overview_text <- c(
  ...
  "Allele Filtered Distinct Assigned Alleles:" = overview_allele_filtered_distinct_assigned_allele_count,
  ...
  "Fully Filtered Distinct Assigned Alleles:" = overview_distinct_assigned_allele_count,
...
overview_total_allele_counts_by_gene <- mbe %>%
  count_alleles(gene, gene_markdown, oxa_family, gene_type, name = "total")

overview_assigned_allele_counts_by_gene <- mbe %>%
  remove_unassigned_bla() %>%
  count_alleles(gene, gene_markdown, oxa_family, name = "assigned")

overview_unassigned_allele_counts_by_gene <- mbe %>%
  remove_assigned_bla() %>%
  count_alleles(gene, gene_markdown, oxa_family, name = "unassigned")

overview_distinct_assigned_allele_counts_by_gene <- mbe %>%
  filter_assigned_bla() %>%
  dplyr::distinct(allele, .keep_all = TRUE) %>%
  count_alleles(gene, gene_markdown, oxa_family, name = "distinct_assigned")

overview_distinct_unassigned_allele_counts_by_gene <- mbe %>%
  filter_unassigned_bla() %>%
  dplyr::distinct(ipg, .keep_all = TRUE) %>%
  count_alleles(gene, gene_markdown, oxa_family, name = "distinct_unassigned")

overview_combined_allele_counts_by_gene <- overview_total_allele_counts_by_gene %>%
  dplyr::left_join(overview_assigned_allele_counts_by_gene,
                   by = c("gene", "gene_markdown", "oxa_family")) %>%
  dplyr::left_join(overview_distinct_assigned_allele_counts_by_gene,
                   by = c("gene", "gene_markdown", "oxa_family")) %>%
  dplyr::left_join(overview_unassigned_allele_counts_by_gene,
                   by = c("gene", "gene_markdown", "oxa_family")) %>%
    dplyr::left_join(overview_distinct_unassigned_allele_counts_by_gene,
                   by = c("gene", "gene_markdown", "oxa_family")) %>%
  dplyr::mutate(across(c(total,assigned,distinct_assigned,unassigned,distinct_unassigned), tidyr::replace_na, 0)) %>%
...
ggplot2::ggsave(filename = file.path(
  graphic_path,
  "acquired_gene_frequency.svg"
),
plot = acquired_gene_frequency_plot)
```

#### Distinct, assigned alleles and families
```{r, eval = FALSE}
# List and count of distinct assigned alleles
overview_distinct_assigned_allele_counts <- mbe %>%
  remove_unassigned_bla() %>%
  count_alleles(gene,
                gene_markdown,
                allele,
...
overview_distinct_assigned_oxa_family_counts <- mbe %>%
  remove_unassigned_bla() %>%
...
overview_isolate_counts_by_species <- mbe %>%
  count_isolates(species, species_markdown) %>%
  mutate(proportion = isolates / length(unique(.env$mbe$biosample)))
...
overview_isolate_counts_by_standardized_country <- mbe %>%
...
overview_isolate_counts_by_region <- mbe %>%
...
overview_isolate_counts_by_collection_year <- mbe %>%
  mutate(
    collected = case_when(
...
overview_isolate_counts_by_collection_period <- mbe %>%
...
# Isolate Host Counts
overview_isolate_counts_by_host <- mbe %>%
  count_isolates(host)
...
```{r, eval = FALSE}
# All genes
overview_combination_allele_list <- mbe %>%
...
# Intrinsic genes
overview_intrinsic_allele_list <- mbe %>%
  filter(gene %in% intrinsic_genes | oxa_family %in% intrinsic_oxa_families) %>%
  determine_combinations()

# Genes of interest overall
overview_combination_intrinsic_allele_counts <-
  overview_intrinsic_allele_list %>%
  count_isolates(combo)

overview_combination_intrinsic_allele_counts %>%
  readr::write_csv(file.path(
    table_path,
      "overview_combination_intrinsic_allele_counts.csv"
  ))
...
```{r, eval = FALSE}
# Presence by Region
carbapenemase_presence_by_region <- mbe %>%
...
# Presence Over Time
carbapenemase_presence_over_time <- mbe %>%
  group_by(period) %>%
  summarize(
    isolates = n_distinct(biosample),
    carbapenemase_alleles = length(allele[subclass == "CARBAPENEM"]),
...
carbapenemase_presence_over_time_since_2000 <- mbe %>%
...
# Allele Counts
carbapenemase_allele_counts <- mbe %>%
...
total_carbapenemase_counts <- mbe %>%
  filter(subclass == "CARBAPENEM") %>%
  count_isolates()
  
total_carbapenemase_counts %>%
  readr::write_lines(file.path(table_path, "carbapenemase_total_isolates.txt"))
...
species_exclusive_allele_overview <- mbe %>%
...
species_exclusive_allele_counts <- mbe %>%
  filter_exclusive_alleles(species) %>%
  count_alleles(species, allele) %>%
  relevel_numeric(species, allele) %>%
  dplyr::arrange(species, allele)
...
### Genes of interest
#### Allele counts and donut plots (intrinsic and acquired)
```{r, eval = FALSE}
for (i in 1:nrow(combinations)) {
  gene_filter <- combinations[[i, "name"]]
  gene_filter_label <- combinations[[i, "label"]]
  
  for (gene_type_value in c("Intrinsic", "Acquired")){
    gene_type_label <- tolower(gene_type_value)
    
    # Filter by `gene` and `gene_type` == `gene_type_value`
    mbe_gene <- mbe %>%
      filter_attribute(gene, gene_filter) %>%
      filter(gene_type == gene_type_value)
    
    if(nrow(mbe_gene) > 0){
      
      # Distinct assigned `allele` frequency
      gene_assigned_type_frequency <- mbe_gene %>%
        remove_unassigned_bla() %>%
        mutate(total_rows = length(unique(mbe_gene$biosample))) %>%
        {if(gene_filter == "blaOXA") count_isolates(., allele, allele_markdown, oxa_family, total_rows)
      ...
      gene_assigned_type_frequency %>%
        readr::write_csv(file.path(table_path, paste0(gene_filter, "_", gene_type_label, "_allele_frequency.csv")))
      
      gene_assigned_type_frequency_plot <-
        gene_assigned_type_frequency %>%
        top_n_across_groups(allele, allele_markdown, count = isolates, n = 10) %>%
        donut_plot(
      ...
      ggplot2::ggsave(filename = file.path(
        graphic_path,
        paste0(gene_filter, "_", gene_type_label, "_allele_frequency_donut.svg")),
        plot = gene_assigned_type_frequency_plot
      )
      
      # Count of <gene_filter> alleles unique to a single species (in the data set)
      gene_species_exclusive_allele_overview <- mbe_gene %>%
        remove_unassigned_bla() %>%
        filter_exclusive_alleles(species) %>%
        dplyr::group_by(species) %>%
        dplyr::summarize(alleles = length(unique(allele))) %>%
      ...
      gene_species_exclusive_allele_overview %>%
        readr::write_csv(file.path(
          table_path,
          paste0(gene_filter, "_", gene_type_label, "_species_exclusive_allele_overview.csv"
          )
        ))
      
      # List (with counts) of <gene_filter> alleles unique to a single species (in the data set)
      gene_species_exclusive_allele_counts <- mbe_gene %>%
        filter_exclusive_alleles(species) %>%
        dplyr::group_by(species) %>%
        mutate(total_rows = length(unique(biosample))) %>%
        {if(gene_filter == "blaOXA") count_isolates(., species, allele, oxa_family, total_rows)
      ...
      gene_species_exclusive_allele_counts %>%
        readr::write_csv(file.path(
          table_path,
          paste0(gene_filter, "_", gene_type_label, "_species_exclusive_allele_counts.csv"
          )
        ))
      
      # Count of isolates with multiple of <gene_filter> by species
      gene_multiples_overview_by_species <- mbe_gene %>%
      ...
      gene_multiples_overview_by_species %>%
        readr::write_csv(file.path(
          table_path,
          paste0(gene_filter, "_", gene_type_label, "_allele_multiples_overview_by_species.csv"
          )
        ))
      
      # List of isolates with multiple of <gene_filter>
      gene_multiples_by_isolate <- mbe_gene %>%
        dplyr::group_by(biosample) %>%
        dplyr::filter(length(unique(allele)) > 1) %>%
        determine_combinations() %>%
        dplyr::distinct(biosample, .keep_all = TRUE) %>%
      ...
      gene_multiples_by_isolate %>%
        readr::write_csv(file.path(
          table_path,
          paste0(gene_filter, "_", gene_type_label, "_allele_multiples_by_isolate.csv")
        ))
      
      # Frequency of distinct combinations of multiple of <gene_filter>
      gene_combination_counts <- mbe_gene %>%
        dplyr::group_by(biosample) %>%
        dplyr::filter(length(unique(allele)) > 1) %>%
        determine_combinations() %>%
        dplyr::ungroup() %>%
      ...
      gene_combination_counts %>%
        readr::write_csv(file.path(
          table_path,
          paste0(gene_filter, "_", gene_type_label, "_allele_combination_counts.csv")
        ))
      
      # Full <gene_filter> `allele` counts and summary graphics by Collection Period
      gene_frequency_by_collection_period <- mbe_gene %>%
        remove_unassigned_bla() %>%
        tidyr::drop_na(period) %>%
        dplyr::group_by(period) %>%
        dplyr::mutate(total_rows = length(unique(biosample))) %>%
        {if(gene_filter == "blaOXA") count_isolates(., period, allele, allele_markdown, oxa_family, total_rows)
      ...
      gene_frequency_by_collection_period %>%
        ...
        readr::write_csv(
          file.path(
            table_path,
            paste0(gene_filter, "_", gene_type_label, "_allele_frequency_by_collection_period.csv"
            )
          )
        )
      ...
      gene_frequency_by_collection_period_rank %>%
        readr::write_csv(
          file.path(
            table_path,
            paste0(gene_filter, "_", gene_type_label, "_allele_frequency_by_collection_period_rank.csv"
            )
          )
        )
      ...
      ggplot2::ggsave(filename = file.path(
        graphic_path,
        paste0(gene_filter, "_", gene_type_label, "_allele_top_5_donut_across_groups_by_collection_period.svg"
        )),
      ...
      ggplot2::ggsave(filename = file.path(
        graphic_path,
        paste0(gene_filter, "_", gene_type_label, "_allele_top_5_donut_overall_by_collection_period.svg"
        )),
        plot = gene_frequency_by_collection_period_overall_plot
      )
      
      # Full <gene_filter> `allele` counts and summary graphics by
      # Collection Period (only for isolates collected since 2000)
      gene_frequency_by_collection_period_2000 <- mbe_gene %>%
        dplyr::filter(year >= 2000) %>%
        remove_unassigned_bla() %>%
        tidyr::drop_na(period_2000) %>%
        dplyr::group_by(period_2000) %>%
        mutate(total_rows = length(unique(biosample))) %>%
        {if(gene_filter == "blaOXA") count_isolates(., period_2000, allele, allele_markdown, oxa_family, total_rows)
      ...
      gene_frequency_by_collection_period_2000 %>%
        ...
        readr::write_csv(file.path(
          table_path,
          paste0(gene_filter, "_", gene_type_label, "_allele_frequency_by_collection_period_2000s.csv"
          )))
      ...
      gene_frequency_by_collection_period_2000_rank %>%
        readr::write_csv(
          file.path(
            table_path,
            paste0(gene_filter, "_", gene_type_label, "_allele_frequency_by_collection_period_2000s_rank.csv"
            )
          )
        )
      ...
      ggplot2::ggsave(filename = file.path(
        graphic_path,
        paste0(gene_filter, "_", gene_type_label, "_allele_top_5_donut_across_groups_by_collection_period_2000s.svg"
        )),
      ...
      ggplot2::ggsave(filename = file.path(
        graphic_path,
        paste0(gene_filter, "_", gene_type_label, "_allele_top_5_donut_overall_by_collection_period_2000s.svg"
        )),
        plot = gene_frequency_by_collection_period_2000_overall_plot
      )
      
      # Full <gene_filter> `allele` counts and top 10 graphic by Region
      gene_frequency_by_region <- mbe_gene %>%
        remove_unassigned_bla() %>%
        tidyr::drop_na(region) %>%
        dplyr::group_by(region) %>%
        mutate(total_rows = length(unique(biosample))) %>%
        {if(gene_filter == "blaOXA") count_isolates(., region, allele, allele_markdown, oxa_family, total_rows)
      ...
      gene_frequency_by_region %>%
        ...
        readr::write_csv(file.path(
          table_path,
          paste0(gene_filter, "_", gene_type_label, "_allele_frequency_by_region.csv")
        ))
      ...
      gene_frequency_by_region_rank %>%
        readr::write_csv(
          file.path(
            table_path,
            paste0(gene_filter, "_", gene_type_label, "_allele_frequency_by_region_rank.csv"
            )
          )
        )
      ...
      ggplot2::ggsave(filename = file.path(
        graphic_path,
        paste0(gene_filter, "_", gene_type_label, "_allele_top_10_donut_overall_by_region.svg"
        )),
      ...
      ggplot2::ggsave(filename = file.path(
        graphic_path,
        paste0(gene_filter, "_", gene_type_label, "_allele_top_5_donut_overall_by_region.svg"
        )),
      ...
      ggplot2::ggsave(filename = file.path(
        graphic_path,
        paste0(gene_filter, "_", gene_type_label, "_allele_top_5_donut_across_groups_by_region.svg"
        )),
        plot = gene_frequency_by_region_across_regions_5_plot
      )
      
      # Full <gene_filter> `allele` counts by Species (50+ alleles)
      gene_frequency_by_species <- mbe_gene %>%
        dplyr::group_by(species) %>%
        dplyr::filter(n() >= 50) %>%
        dplyr::ungroup() %>%
        remove_unassigned_bla() %>%
        ...
        dplyr::group_by(species) %>%
        mutate(total_rows = length(unique(biosample))) %>%
        {if(gene_filter == "blaOXA") count_isolates(., species, species_markdown, allele, allele_markdown, oxa_family, total_rows)
      ...
      gene_frequency_by_species %>%
        ...
        readr::write_csv(file.path(
          table_path,
          paste0(gene_filter, "_", gene_type_label, "_allele_frequency_by_species.csv")
        ))
      ...
      gene_frequency_by_species_rank %>%
        readr::write_csv(
          file.path(
            table_path,
            paste0(gene_filter, "_", gene_type_label, "_allele_frequency_by_species_rank.csv"
            )
          )
        )
      ...
      ggplot2::ggsave(filename = file.path(
        graphic_path,
        paste0(gene_filter, "_", gene_type_label, "_allele_top_5_donut_overall_by_species.svg"
        )),
      ...
      gene_frequency_by_species_across_groups_plot <-
        ...
        top_n_across_groups(allele,allele_markdown, count = isolates, n = 5) %>%
        donut_plot_panel(
          fill = allele_markdown,
          facet = species_markdown,
          counts = isolates,
      ...
      ggplot2::ggsave(filename = file.path(
        graphic_path,
        paste0(gene_filter, "_", gene_type_label, "_allele_top_5_donut_across_groups_by_species.svg"
        )),
...
if("OXA" %in% combinations[["name"]]) {
  ...
  for (gene_type_value in c("Intrinsic", "Acquired")){
    ...
    # Filter by `gene_type` == `gene_type_value`
    mbe_gene <- mbe %>%
      filter_attribute(gene, gene_filter) %>%
      filter(gene_type == gene_type_value)
    
    # Verify rows exist to process
    if(nrow(mbe_gene) > 0){
      ...
      gene_assigned_type_frequency %>%
        readr::write_csv(file.path(
          table_path,
          paste0(gene_type_label, "_OXA_family_frequency.csv")
        ))
      ...
      ggplot2::ggsave(filename = file.path(
        graphic_path,
        paste0(gene_type_label,"_OXA_family_frequency_donut.svg")),
        plot = gene_assigned_type_frequency_plot
      )
      ...
      gene_species_exclusive_allele_overview %>%
        readr::write_csv(file.path(
          table_path,
          paste0(gene_type_label, "_OXA_family_species_exclusive_allele_overview.csv")
        ))
      ...
      gene_species_exclusive_allele_counts %>%
        readr::write_csv(file.path(
          table_path,
          paste0(gene_type_label, "_OXA_family_species_exclusive_allele_counts.csv")
        ))
      ...
      gene_multiples_overview_by_species %>%
        readr::write_csv(file.path(
          table_path,
          paste0(gene_type_label, "_OXA_family_multiples_overview_by_species.csv")
        ))
      ...
      gene_multiples_by_isolate %>%
        readr::write_csv(file.path(
          table_path,
          paste0(gene_type_label, "_OXA_family_multiples_by_isolate.csv")
        ))
      ...
      gene_combination_counts %>%
        readr::write_csv(file.path(
          table_path,
          paste0(gene_type_label, "_OXA_family_combination_counts.csv")
        ))
      ...
      gene_frequency_by_collection_period %>%
        ...
        readr::write_csv(
          file.path(
            table_path,
            paste0(gene_type_label, "_OXA_family_frequency_by_collection_period.csv")
          )
        )
      ...
      gene_frequency_by_collection_period_rank %>%
        readr::write_csv(
          file.path(
            table_path,
            paste0(gene_type_label, "_OXA_family_frequency_by_collection_period_rank.csv")
        ))  
      ...
      ggplot2::ggsave(filename = file.path(
        graphic_path,
        paste0(gene_type_label, "_OXA_family_top_5_donut_across_groups_by_collection_period.svg")
      ),
      plot = gene_frequency_by_collection_period_across_groups_plot
      )
      ...
      ggplot2::ggsave(filename = file.path(
        graphic_path,
        paste0(gene_type_label, "_OXA_family_top_5_donut_overall_by_collection_period.svg")
      ),
      plot = gene_frequency_by_collection_period_overall_plot
      )
      ...
      gene_frequency_by_collection_period_2000 %>%
        ...
        readr::write_csv(file.path(
          table_path,
          paste0(gene_type_label, "_OXA_family_frequency_by_collection_period_2000s.csv")
        ))
      ...
      gene_frequency_by_collection_period_rank_2000 %>%
        readr::write_csv(
          file.path(
            table_path,
            paste0(gene_type_label, "_OXA_family_frequency_by_collection_period_2000s_rank.csv")
        )) 
      ...
      ggplot2::ggsave(filename = file.path(
        graphic_path,
        paste0(gene_type_label, "_OXA_family_top_5_donut_across_groups_by_collection_period_2000s.svg")
      ),
      plot = gene_frequency_by_collection_period_2000_across_groups_plot
      )
      ...
      ggplot2::ggsave(filename = file.path(
        graphic_path,
        paste0(gene_type_label, "_OXA_family_top_5_donut_overall_by_collection_period_2000s.svg")
      ),
      plot = gene_frequency_by_collection_period_2000_overall_plot
      )
      ...
      gene_frequency_by_region %>%
        ...
        readr::write_csv(file.path(
          table_path,
          paste0(gene_type_label, "_OXA_family_frequency_by_region.csv")
        ))
      ...
      gene_frequency_by_region_rank %>%
        readr::write_csv(
          file.path(
            table_path,
            paste0(gene_type_label, paste0("_OXA_family_frequency_by_region_rank.csv")
            )
          )
        )   
      ...
      ggplot2::ggsave(filename = file.path(
        graphic_path,
        paste0(gene_type_label, "_OXA_family_top_10_donut_overall_by_region.svg")
      ),
      plot = gene_frequency_by_region_overall_10_plot
      )
      ...
      ggplot2::ggsave(filename = file.path(
        graphic_path,
        paste0(gene_type_label, "_OXA_family_top_5_donut_overall_by_region.svg")
      ),
      plot = gene_frequency_by_region_overall_5_plot
      )
      ...
      ggplot2::ggsave(filename = file.path(
        graphic_path,
        paste0(gene_type_label, "_OXA_family_top_5_donut_across_groups_by_region.svg")
      ),
      plot = gene_frequency_by_region_across_regions_5_plot
      )
      ...
      gene_frequency_by_species %>%
        ...
        readr::write_csv(file.path(
          table_path, paste0(gene_type_label, "_OXA_family_frequency_by_species.csv")
        ))
      ...
      gene_frequency_by_species_rank %>%
        readr::write_csv(
          file.path(
            table_path,
            paste0(gene_type_label, "_OXA_family_frequency_by_species_rank.csv")
        ))   
      ...
      ggplot2::ggsave(filename = file.path(
        graphic_path,
        paste0(gene_type_label, "_OXA_family_top_5_donut_overall_by_species.svg")
      ),
      plot = gene_frequency_by_species_overall_plot
      )
      ...
      ggplot2::ggsave(filename = file.path(
        graphic_path,
        paste0(gene_type_label, "_OXA_family_top_5_donut_across_groups_by_species.svg")
      ),
      plot = gene_frequency_by_species_across_groups_plot
      )
    }
...
for (gene_name in genes_of_interest) {
  ...
  mbe_goi_unassigned_allele_list <- mbe %>%
    ...
    {if(gene_name == "blaOXA") dplyr::filter(., oxa_family %in% intrinsic_oxa_families) else .} %>%
    tidyr::drop_na(ipg) %>%
    dplyr::group_by(ipg) %>%
    dplyr::mutate(ipg_accession = head(ipg_accession, n = 1L)) %>%
    {if(gene_name == "blaOXA") count_isolates(., ipg, ipg_accession, gene, oxa_family) 
      else count_isolates(., ipg, gene, ipg_accession)} %>%
    dplyr::arrange(desc(isolates))
  
  mbe_goi_unassigned_allele_list %>%
    readr::write_csv(file.path(
      table_path,
      paste0("mbe_unassigned_intrinsic_", gene_name, "_alleles.csv")
    ))
  
  if(gene_name == "blaOXA"){
    mbe_goi_unassigned_family_list <- mbe %>%
      remove_assigned_bla() %>%
      dplyr::filter(oxa_family %in% intrinsic_oxa_families) %>%
      tidyr::drop_na(ipg) %>%
      count_isolates(oxa_family) %>%
      dplyr::arrange(desc(isolates))
    
    mbe_goi_unassigned_family_list %>%
      readr::write_csv(file.path(
        table_path,
        paste0("mbe_unassigned_intrinsic_", gene_name, "_families.csv")
      ))   
  }
  ...
  ggplot2::ggsave(filename = file.path(
    graphic_path,
    paste0("mbe_unassigned_intrinsic_top10_", gene_name, "_alleles_donut.svg")),
    plot = mbe_goi_unassigned_allele_plot
  )
  

  # <gene_name> Allele Counts by PDS Cluster
  gene_filter_alleles_by_pds <- mbe %>%
    filter_assigned_bla() %>%
    relevel_numeric(allele) %>%
    {if(gene_name == "blaOXA") dplyr::filter(., oxa_family %in% intrinsic_oxa_families)
      else filter_attribute(., allele, gene_name)} %>%
    group_by(pds)
  
  gene_filter_alleles_by_pds %>%
    arrange(allele) %>%
    dplyr::summarize(count = n_distinct(biosample), alleles = paste(unique(allele), collapse = ", ")) %>%
    arrange(desc(count)) %>%
    readr::write_csv(file.path(
      table_path, paste0(gene_name, "_intrinsic_allele_list_by_pds.csv")
    ))
  
  gene_filter_alleles_by_pds %>%
    summarize(alleles = n_distinct(allele)) %>%
    arrange(desc(alleles)) %>%
    readr::write_csv(file.path(
      table_path, paste0(gene_name, "_intrinsic_allele_counts_by_pds.csv")
    ))
  
  # <gene_name> Allele Lists and Counts by MLST
  gene_filter_alleles_by_mlst <- mbe %>%
    filter_assigned_bla() %>%
    relevel_numeric(allele) %>%
    {if(gene_name == "blaOXA") dplyr::filter(., oxa_family %in% intrinsic_oxa_families)
      else filter_attribute(., allele, gene_name)} %>%
  ...
  gene_filter_alleles_by_mlst %>%
    arrange(allele) %>%
    dplyr::summarize(count = n_distinct(biosample), alleles = paste(unique(allele), collapse = ", ")) %>%
    arrange(desc(count)) %>%
    readr::write_csv(file.path(
      table_path, paste0(gene_name, "_intrinsic_allele_list_by_mlst.csv")
    ))
  ...
  gene_filter_alleles_by_mlst %>%
    ...
    arrange(desc(alleles)) %>%
    readr::write_csv(file.path(
      table_path, paste0(gene_name, "_intrinsic_allele_counts_by_mlst.csv")
    ))

    # <gene_name> MLST Lists and Counts by Allele
  gene_filter_mlst_by_allele <- mbe %>%
    filter_assigned_bla() %>%
    relevel_numeric(mlst) %>%
    {if(gene_name == "blaOXA") dplyr::filter(., oxa_family %in% intrinsic_oxa_families)
      else filter_attribute(., allele, gene_name)} %>%
    {if(gene_name == "blaOXA") group_by(., allele, oxa_family) 
      else group_by(., allele)} 
  
  gene_filter_mlst_by_allele %>%
    arrange(mlst) %>%
    tidyr::drop_na(mlst) %>%
    dplyr::summarize(count = n_distinct(biosample), alleles = paste(unique(mlst), collapse = ", ")) %>%
    arrange(desc(count)) %>%
    readr::write_csv(file.path(
      table_path, paste0(gene_name, "_mlst_list_by_intrinsic_allele.csv")
    ))
  
  gene_filter_mlst_by_allele %>%
    summarize(sequence_types = n_distinct(mlst)) %>%
    arrange(desc(sequence_types)) %>%
    readr::write_csv(file.path(
      table_path, paste0(gene_name, "_mlst_counts_by_intrinsic_allele.csv")
    ))  
  
      # <gene_name> PDS Lists and Counts by Allele
  gene_filter_pds_by_allele <- mbe %>%
    filter_assigned_bla() %>%
    relevel_numeric(pds) %>%
    {if(gene_name == "blaOXA") dplyr::filter(., oxa_family %in% intrinsic_oxa_families)
      else filter_attribute(., allele, gene_name)} %>%
  ...
  gene_filter_pds_by_allele %>%
    arrange(pds) %>%
    tidyr::drop_na(pds) %>%
    dplyr::summarize(count = n_distinct(biosample), alleles = paste(unique(pds), collapse = ", ")) %>%
    arrange(desc(count)) %>%
    readr::write_csv(file.path(
      table_path, paste0(gene_name, "_pds_list_by_intrinsic_allele.csv")
    ))
  
  gene_filter_pds_by_allele %>%
    summarize(sequence_types = n_distinct(pds)) %>%
    arrange(desc(sequence_types)) %>%
    readr::write_csv(file.path(
      table_path, paste0(gene_name, "_pds_counts_by_intrinsic_allele.csv")
    )) 
}
```

# Acquired Gene Counts

```{r, eval = FALSE}
acquired_gene_alleles <- mbe %>%
  filter(gene_type == "Acquired")

acquired_gene_counts <- acquired_gene_alleles %>%
  count_isolates(gene, oxa_family) %>%
  dplyr::arrange(desc(isolates)) %>%
  mutate(prop = isolates / (mbe %>% distinct(biosample) %>% nrow()))

readr::write_csv(acquired_gene_counts, file.path(table_path, "overview_acquired_gene_counts.csv"))


acquired_allele_counts <- acquired_gene_alleles %>%
  count_isolates(allele, oxa_family) %>%
  dplyr::arrange(desc(isolates)) %>%
  mutate(prop = isolates / (mbe %>% distinct(biosample) %>% nrow()))

readr::write_csv(acquired_allele_counts, file.path(table_path, "overview_acquired_allele_counts.csv"))

```


# Deduplicated and Allele Presence Data
...
mlst_region <- mbe %>%
  tidyr::drop_na(region) %>%
  count_isolates(region, mlst)

readr::write_csv(mlst_region, file.path(table_path, "overview_mlst_region.csv"))

# MLST Ranks by Region
mlst_region_ranks <- mlst_region %>%
  dplyr::group_by(region) %>%
  dplyr::mutate(rank = dense_rank(desc(isolates))) %>%
  dplyr::select(-isolates) %>%
  tidyr::pivot_wider(names_from = region, values_from = rank)

readr::write_csv(mlst_region_ranks, file.path(table_path, "overview_mlst_region_ranks.csv"))

# MLST Ranks by Collection Period (Post 2000)
mlst_period_2000 <- mbe %>%
  tidyr::drop_na(period_2000) %>%
  count_isolates(period_2000, mlst)

readr::write_csv(mlst_period_2000, file.path(table_path, "overview_mlst_collection_period_2000.csv"))

# MLST Ranks by Collection Period (Post 2000)
mlst_period_2000_ranks <- mlst_period_2000 %>%
  dplyr::group_by(period_2000) %>%
  dplyr::mutate(rank = dense_rank(desc(isolates))) %>%
  dplyr::select(-isolates) %>%
  tidyr::pivot_wider(names_from = period_2000, values_from = rank)

readr::write_csv(mlst_period_2000_ranks, file.path(table_path, "overview_mlst_collection_period_2000_ranks.csv"))
...
for (i in 1:nrow(combinations)) {
  ...
  mlst_deduplicated_raw <- mbe %>%
    filter_assigned_bla() %>%
    determine_combinations() %>%
    {if(gene_filter == "blaOXA") dplyr::filter(., oxa_family %in% intrinsic_oxa_families)
      else filter_attribute(., allele, gene_filter)} %>%
    tidyr::drop_na(mlst)
  
  mlst_deduplicated_biosamples <- mlst_deduplicated_raw %>%
    distinct(mlst, combo, .keep_all = TRUE) %>%
    pull(biosample)
  
  mlst_deduplicated <- mlst_deduplicated_raw %>%
    filter(biosample %in% mlst_deduplicated_biosamples)
  
  mlst_deduplicated_frequency <- mlst_deduplicated %>%
    {if(gene_filter == "blaOXA") count_isolates(., allele, allele_markdown, oxa_family) 
      else count_isolates(., allele, allele_markdown)} %>%
    mutate(proportion = isolates / (mlst_deduplicated %>% distinct(mlst, combo) %>% nrow()) ) %>%
    mutate(rank = dplyr::dense_rank(desc(isolates))) %>%
    left_join(mbe %>%
                filter_assigned_bla() %>%
                {if(gene_filter == "blaOXA") dplyr::filter(., oxa_family %in% intrinsic_oxa_families)
  ...
  mlst_deduplicated_frequency %>%
    readr::write_csv(file.path(
      table_path,
      paste0(gene_filter, "_intrinsic_allele_frequency_mbe_mlst_deduplicated.csv")
    ))
  ...
  ggplot2::ggsave(filename = file.path(
    graphic_path,
    paste0(gene_filter, "_intrinsic_allele_frequency_mbe_mlst_deduplicated_donut.svg")),
    plot = mlst_deduplicated_frequency_plot
  ) 
  
  # SNP Cluster Deduplicated - Assigned allele frequency
  pds_deduplicated_raw <- mbe %>%
    filter_assigned_bla() %>%
    determine_combinations() %>%
    {if(gene_filter == "blaOXA") dplyr::filter(., oxa_family %in% intrinsic_oxa_families)
      else filter_attribute(., allele, gene_filter)} %>%
  ...
  pds_deduplicated_biosamples <- pds_deduplicated_raw %>%
    distinct(pds, combo, .keep_all = TRUE) %>%
    pull(biosample)
  
  pds_deduplicated <- pds_deduplicated_raw %>%
    filter(biosample %in% pds_deduplicated_biosamples)
  
  
  pds_deduplicated_frequency <- pds_deduplicated %>%
    {if(gene_filter == "blaOXA") count_isolates(., allele, allele_markdown, oxa_family) 
      else count_isolates(., allele, allele_markdown)} %>%
    mutate(proportion = isolates / (pds_deduplicated %>% distinct(pds, combo) %>% nrow()) ) %>%
    mutate(rank = dplyr::dense_rank(desc(isolates))) %>%
    left_join(mbe %>%
                filter_assigned_bla() %>%
                {if(gene_filter == "blaOXA") dplyr::filter(., oxa_family %in% intrinsic_oxa_families)
  ...
  pds_deduplicated_frequency %>%
    readr::write_csv(file.path(
      table_path,
      paste0(gene_filter, "_intrinsic_allele_frequency_mbe_pds_deduplicated.csv")
    ))
  ...
  ggplot2::ggsave(filename = file.path(
    graphic_path,
    paste0(gene_filter, "_intrinsic_allele_frequency_mbe_pds_deduplicated_donut.svg")),
  ...
  pds_unclustered_frequency <- pds_unclustered %>%
    ...
    mutate(proportion = isolates / length(unique(pds_unclustered$biosample))) %>%
    mutate(rank = dplyr::dense_rank(desc(isolates))) %>%
    left_join(mbe %>%
                filter_assigned_bla() %>%
                {if(gene_filter == "blaOXA") dplyr::filter(., oxa_family %in% intrinsic_oxa_families)
  ...
  pds_unclustered_frequency %>%
    readr::write_csv(file.path(
      table_path,
      paste0(gene_filter, "_intrinsic_allele_frequency_mbe_pds_unclustered.csv")
    ))
  ...
  ggplot2::ggsave(filename = file.path(
    graphic_path,
    paste0(gene_filter, "_intrinsic_allele_frequency_mbe_pds_unclustered_donut.svg")),
  ...
  pds_deduplicated_unclustered_frequency <- pds_deduplicated_unclustered %>%
    ...
    mutate(proportion = isolates / (((pds_deduplicated %>% distinct(pds, combo) %>% nrow()) ) + (length(unique(pds_unclustered$biosample))))) %>%
    mutate(rank = dplyr::dense_rank(desc(isolates))) %>%
    left_join(mbe %>%
                filter_assigned_bla() %>%
                {if(gene_filter == "blaOXA") dplyr::filter(., oxa_family %in% intrinsic_oxa_families)
  ...
  pds_deduplicated_unclustered_frequency %>%
    readr::write_csv(file.path(
      table_path,
      paste0(gene_filter, "_intrinsic_allele_frequency_mbe_pds_deduplicated_and_unclustered.csv")
    ))
  ...
  ggplot2::ggsave(filename = file.path(
    graphic_path,
    paste0(gene_filter, "_intrinsic_allele_frequency_mbe_pds_deduplicated_and_unclustered_donut.svg")),
  ...
  combined_deduplicated_ranks <- mbe %>%
    ...
    {if(gene_filter == "blaOXA") count_isolates(., allele, oxa_family) 
      else count_isolates(., allele)} %>%
    dplyr::mutate(full_data_rank = dplyr::dense_rank(desc(isolates))) %>%
    dplyr::mutate(full_prop = isolates / mbe %>% remove_unassigned_bla() %>% distinct(biosample) %>% nrow()) %>%
    {if(gene_filter == "blaOXA") dplyr::select(., allele, oxa_family, full_data_rank, full_prop) 
      else dplyr::select(., allele, full_data_rank, full_prop)} %>%
    dplyr::full_join(
      mlst_deduplicated_frequency %>%
        dplyr::select(allele, mlst_deduplicated_rank = rank, mlst_prop = proportion),
      by = "allele") %>%
    dplyr::full_join(
      pds_deduplicated_frequency %>%
        dplyr::select(allele, pds_deduplicated_rank = rank, pds_dedupe_prop = proportion),
      by = "allele") %>%
    dplyr::full_join(
      pds_unclustered_frequency %>%
        dplyr::select(allele, pds_unclustered_rank = rank, pds_unclust_prop = proportion),
      by = "allele") %>%
    dplyr::full_join(
      pds_deduplicated_unclustered_frequency %>%
        dplyr::select(allele, pds_deduplicated_unclustered_rank = rank, pds_dedupe_unclust_prop = proportion),
      by = "allele") %>%
    mutate(
      mlst_delta = full_data_rank - mlst_deduplicated_rank,
      pds_dedupe_delta = full_data_rank - pds_deduplicated_rank,
      pds_unclust_delta = full_data_rank - pds_unclustered_rank,
      pds_dedupe_unclust_delta = full_data_rank - pds_deduplicated_unclustered_rank 
    ) %>%
    mutate(
      abs_max_delta = pmax(abs(mlst_delta), abs(pds_dedupe_delta), abs(pds_unclust_delta), abs(pds_dedupe_unclust_delta), na.rm = TRUE) ,
      max_rank = pmin(full_data_rank, mlst_deduplicated_rank, pds_deduplicated_rank, pds_unclustered_rank, pds_deduplicated_unclustered_rank, na.rm = TRUE),
      min_rank = pmax(full_data_rank, mlst_deduplicated_rank, pds_deduplicated_rank, pds_unclustered_rank, pds_deduplicated_unclustered_rank, na.rm = TRUE),
      max_prop = pmax(full_prop, mlst_prop, pds_dedupe_prop, pds_unclust_prop, pds_dedupe_unclust_prop, na.rm = TRUE),
    )
  
  combined_deduplicated_ranks %>%
    readr::write_csv(file.path(
      table_path,
      paste0(gene_filter, "_intrinsic_allele_frequency_mbe_mlst_pds_deduplicated_ranks.csv")
    ))
}
```

## Allele Presence and Absence - MLST & PDS Cluster - MBE
```{r, eval = FALSE}
for (i in 1:nrow(combinations)) {
  gene_filter <- combinations[[i, "name"]]
  gene_filter_label <- combinations[[i, "label"]]
  ...
  mlst_presence_frequency <- mlst_presence %>%
    {if(gene_filter == "blaOXA") count_isolates(., allele, allele_markdown, oxa_family) 
      else count_isolates(., allele, allele_markdown)} %>%
    mutate(proportion = isolates / length(unique(mlst_presence$mlst))) %>%
    mutate(rank = dplyr::dense_rank(desc(isolates))) %>%
    left_join(mbe %>%
                filter_assigned_bla() %>%
                {if(gene_filter == "blaOXA") dplyr::filter(., oxa_family %in% intrinsic_oxa_families)
  ...
  mlst_presence_frequency %>%
    readr::write_csv(file.path(
      table_path,
      paste0(gene_filter, "_intrinisc_allele_frequency_mbe_mlst_presence.csv")
    ))
  
  mlst_presence_frequency_plot <-
    mlst_presence_frequency %>%
    top_n_across_groups(allele,
                        allele_markdown,
                        count = isolates,
  ...
  ggplot2::ggsave(filename = file.path(
    graphic_path,
    paste0(gene_filter, "_intrinsic_allele_frequency_mbe_mlst_presence_donut.svg")),
    plot = mlst_presence_frequency_plot
  ) 
  ...
  pds_presence_frequency <- pds_presence %>%
    {if(gene_filter == "blaOXA") count_isolates(., allele, allele_markdown, oxa_family) 
      else count_isolates(., allele, allele_markdown)} %>%
    mutate(proportion = isolates / length(unique(pds_presence$pds))) %>%
    mutate(rank = dplyr::dense_rank(desc(isolates))) %>%
    left_join(mbe %>%
                filter_assigned_bla() %>%
                {if(gene_filter == "blaOXA") dplyr::filter(., oxa_family %in% intrinsic_oxa_families)
  ...
  pds_presence_frequency %>%
    readr::write_csv(file.path(
      table_path,
      paste0(gene_filter, "_intrinsic_allele_frequency_mbe_pds_presence.csv")
    ))
  
  pds_presence_frequency_plot <-
    pds_presence_frequency %>%
    top_n_across_groups(allele,
                        allele_markdown,
                        count = isolates,
  ...
  ggplot2::ggsave(filename = file.path(
    graphic_path,
    paste0(gene_filter, "_intrinsic_allele_frequency_mbe_pds_presence_donut.svg")),
    plot = pds_presence_frequency_plot
  ) 
  ...
  pds_unclustered_frequency <- pds_unclustered %>%
    ...
    mutate(proportion = isolates / length(unique(mlst_presence$biosample))) %>%
    mutate(rank = dplyr::dense_rank(desc(isolates))) %>%
    left_join(mbe %>%
                filter_assigned_bla() %>%
                {if(gene_filter == "blaOXA") dplyr::filter(., oxa_family %in% intrinsic_oxa_families)
  ...
  # SNP Cluster Deduplicated + Unclustered - Assigned allele frequency
  pds_presence_unclustered <- pds_presence %>%
    bind_rows(pds_unclustered)
  
  pds_presence_unclustered_frequency <- pds_presence_unclustered %>%
    {if(gene_filter == "blaOXA") count_isolates(., allele, allele_markdown, oxa_family) 
      else count_isolates(., allele, allele_markdown)} %>%
    mutate(proportion = isolates / (length(unique(pds_presence_unclustered$pds)) + (pds_presence_unclustered %>% filter(is.na(pds)) %>% distinct(biosample) %>% nrow()))) %>%
    mutate(rank = dplyr::dense_rank(desc(isolates))) %>%
    left_join(mbe %>%
                filter_assigned_bla() %>%
                {if(gene_filter == "blaOXA") dplyr::filter(., oxa_family %in% intrinsic_oxa_families)
  ...
  pds_presence_unclustered_frequency %>%
    readr::write_csv(file.path(
      table_path,
      paste0(gene_filter, "_intrinsic_allele_frequency_mbe_pds_presence_and_unclustered.csv")
    ))
  
  pds_presence_unclustered_frequency_plot <-
    pds_presence_unclustered_frequency %>%
    top_n_across_groups(allele,
                        allele_markdown,
                        count = isolates,
  ...
  ggplot2::ggsave(filename = file.path(
    graphic_path,
    paste0(gene_filter, "_intrinsic_allele_frequency_mbe_pds_presence_and_unclustered_donut.svg")),
    plot = pds_presence_unclustered_frequency_plot
  )
# Comparison of Full and Deduplicated Data Sets

  combined_presence_ranks <- mbe %>%
  remove_unassigned_bla() %>%
    {if(gene_filter == "blaOXA") dplyr::filter(., oxa_family %in% intrinsic_oxa_families)
      else filter_attribute(., allele, gene_filter)} %>%
  {if(gene_filter == "blaOXA") count_isolates(., allele, oxa_family) 
  ...
  dplyr::full_join(
    mlst_presence_frequency %>%
      dplyr::select(allele, mlst_presence_rank = rank, mlst_prop = proportion),
    by = "allele") %>%
  dplyr::full_join(
    pds_presence_frequency %>%
      dplyr::select(allele, pds_presence_rank = rank, pds_presence_prop = proportion),
    by = "allele") %>%
  dplyr::full_join(
    pds_unclustered_frequency %>%
      dplyr::select(allele, pds_unclustered_rank = rank, pds_unclust_prop = proportion),
  ...
  dplyr::full_join(
    pds_presence_unclustered_frequency %>%
      dplyr::select(allele, pds_presence_unclustered_rank = rank, pds_presence_unclust_prop = proportion),
    by = "allele") %>%
  mutate(
    mlst_delta = full_data_rank - mlst_presence_rank,
    pds_presence_delta = full_data_rank - pds_presence_rank,
    pds_unclust_delta = full_data_rank - pds_unclustered_rank,
    pds_presence_unclust_delta = full_data_rank - pds_presence_unclustered_rank 
  ) %>%
  mutate(
    abs_max_delta = pmax(abs(mlst_delta), abs(pds_presence_delta), abs(pds_unclust_delta), abs(pds_presence_unclust_delta), na.rm = TRUE) ,
    max_rank = pmin(full_data_rank, mlst_presence_rank, pds_presence_rank, pds_unclustered_rank, pds_presence_unclustered_rank, na.rm = TRUE),
    min_rank = pmax(full_data_rank, mlst_presence_rank, pds_presence_rank, pds_unclustered_rank, pds_presence_unclustered_rank, na.rm = TRUE),
    max_prop = pmax(full_prop, mlst_prop, pds_presence_prop, pds_unclust_prop, pds_presence_unclust_prop, na.rm = TRUE),
  )
  
  combined_presence_ranks %>%
    readr::write_csv(file.path(
      table_path,
      paste0(gene_filter, "_intrinsic_allele_frequency_mbe_mlst_pds_presence_ranks.csv")
    ))
}
```
...
```{r, eval = FALSE}
## Counts by Region
overview_bioproject_list_by_region <- mbe %>%
  ...
  overview_bioproject_summary_by_region <- mbe %>%
  count_isolates(region) %>%
  left_join(
    overview_bioproject_list_by_region %>%
  ...
  overview_bioproject_distinct_count <- mbe %>%
    distinct(bioproject, .keep_all = TRUE) %>%
    count_isolates()
  
  overview_bioproject_distinct_count %>%
    readr::write_lines(file.path(table_path, "overview_bioproject_distinct_count.txt"))  
...
for (i in 1:nrow(combinations)) {
  ...
  bioproject_deduplicated <- mbe %>%
    filter_assigned_bla() %>%
    determine_combinations() %>%
    {if(gene_filter == "blaOXA") dplyr::filter(., oxa_family %in% intrinsic_oxa_families)
      else filter_attribute(., allele, gene_filter)} %>%
    tidyr::drop_na(bioproject) %>%
    distinct(bioproject, combo, .keep_all = TRUE)
  
  bioproject_deduplicated_frequency <- bioproject_deduplicated %>%
    {if(gene_filter == "blaOXA") count_isolates(., allele, allele_markdown, oxa_family) 
      else count_isolates(., allele, allele_markdown)} %>%
    mutate(proportion = isolates / (bioproject_deduplicated %>% distinct(bioproject, combo) %>% nrow()) ) %>%
    mutate(rank = dplyr::dense_rank(desc(isolates))) %>%
    full_join(mbe %>%
                filter_assigned_bla() %>%
                {if(gene_filter == "blaOXA") dplyr::filter(., oxa_family %in% intrinsic_oxa_families)
  ...
  bioproject_deduplicated_frequency %>%
    readr::write_csv(file.path(
      table_path,
      paste0(gene_filter, "_intrinsic_allele_frequency_isolates_bioproject_deduplicated.csv")
    ))
  ...
  ggplot2::ggsave(filename = file.path(
    graphic_path,
    paste0(gene_filter, "_intrinsic_allele_frequency_isolates_bioproject_deduplicated_donut.svg")),
    plot = bioproject_deduplicated_frequency_plot
  ) 
}
```

## Allele Presence and Absence - BioProject - Isolates Browser
```{r, eval = FALSE}
for (i in 1:nrow(combinations)) {
  gene_filter <- combinations[[i, "name"]]
  gene_filter_label <- combinations[[i, "label"]]
  ...
  bioproject_presence_frequency <- bioproject_presence %>%
    {if(gene_filter == "blaOXA") count_isolates(., allele, allele_markdown, oxa_family) 
      else count_isolates(., allele, allele_markdown)} %>%
    mutate(proportion = isolates / length(unique(bioproject_presence$biosample))) %>%
    mutate(rank = dplyr::dense_rank(desc(isolates))) %>%
    full_join(mbe %>%
                filter_assigned_bla() %>%
                {if(gene_filter == "blaOXA") dplyr::filter(., oxa_family %in% intrinsic_oxa_families)
  ...
  bioproject_presence_frequency %>%
    readr::write_csv(file.path(
      table_path,
      paste0(gene_filter, "_intrinsic_allele_frequency_isolates_bioproject_presence.csv")
    ))
  
  bioproject_presence_frequency_plot <-
    bioproject_presence_frequency %>%
    top_n_across_groups(allele,
                        allele_markdown,
                        count = isolates,
  ...
  ggplot2::ggsave(filename = file.path(
    graphic_path,
    paste0(gene_filter, "_intrinsic_allele_frequency_isolates_bioproject_presence_donut.svg")),
    plot = bioproject_presence_frequency_plot
  ) 
}
```
...
for (i in 1:nrow(combinations)) {
  ...
  mbe %>%
    ...
    readr::write_lines(file.path(table_path,
                                 paste0("distinct_assigned_intrinsic_", gene_filter, "_proteins.txt")))  
  ...
  mbe %>%
    ...
    dplyr::pull(protein) %>%
    readr::write_lines(file.path(table_path,
                                 paste0("distinct_unassigned_intrinsic_", gene_filter, "_proteins.txt")))
}
