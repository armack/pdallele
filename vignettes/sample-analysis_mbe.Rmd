...
mbe <- mbe_raw %>%
  filter_microbigge(species = species_of_interest,
                    ...
                    deduplicate = TRUE) %>%
  filter_required_alleles(genes = genes_of_interest, family = chromosomal_oxa_family) %>%
...
overview_distinct_assigned_allele_counts %>%
  dplyr::select(gene, allele, oxa_family, gene_type, alleles) %>%
...
overview_distinct_assigned_oxa_family_counts <- mbe %>%
  ...
  count_alleles(gene, gene_markdown, oxa_family, oxa_family_markdown, gene_type) %>%
  relevel_numeric(oxa_family) %>%
  dplyr::arrange(oxa_family)

overview_distinct_assigned_oxa_family_counts %>%
  dplyr::select(gene, oxa_family, gene_type, alleles) %>%
...
for (gene_name in genes_of_interest) {
  ...
  if(gene_name == "OXA"){
    ...
    mbe_goi_unassigned_family_list %>%
      readr::write_csv(file.path(
        table_path,
        paste0("mbe_unassigned_", gene_name, "_families.csv")
      ))   
  }
  ...
  gene_filter_allele_counts_pds <- mbe %>%
    filter_assigned_bla() %>%
    filter_attribute(allele, gene_name) %>%
    group_by(pds) %>%
  ...
  gene_filter_allele_counts_pds %>%
    readr::write_csv(file.path(
      table_path, paste0(gene_name, "_allele_counts_by_pds.csv")
    ))
  ...
  gene_filter_allele_counts_mlst %>%
    readr::write_csv(file.path(
      table_path, paste0(gene_name, "_allele_counts_by_mlst.csv")
    ))
  ...
  gene_filter_mlst_counts_allele <- mbe %>%
    filter_assigned_bla() %>%
    filter_attribute(allele, gene_name) %>%
    {if(gene_name == "OXA") group_by(., allele, oxa_family) 
  ...
  gene_filter_mlst_counts_allele %>%
    readr::write_csv(file.path(
      table_path, paste0(gene_name, "_mlst_counts_by_allele.csv")
    ))  
  ...
  gene_filter_pds_counts_allele %>%
    readr::write_csv(file.path(
      table_path, paste0(gene_name, "_pds_counts_by_allele.csv")
    ))  
}
```
...
mlst_period_2000 <- mbe %>%
  dplyr::left_join(isolates %>% distinct(biosample, period, period_2000), by = "biosample") %>%
  tidyr::drop_na(period_2000) %>%
...
# Comparison of Full and Deduplicated Data Sets
  ...
  {if(gene_filter == "OXA") dplyr::select(., allele, oxa_family, full_data_rank, full_prop) 
...
for (i in 1:nrow(combinations)) {
  ...
  mlst_presence_frequency %>%
    readr::write_csv(file.path(
      table_path,
      paste0(gene_filter, "_allele_frequency_mbe_mlst_presence.csv")
    ))
  ...
  ggplot2::ggsave(filename = file.path(
    graphic_path,
    paste0(gene_filter, "_allele_frequency_mbe_mlst_presence_donut.svg")),
  ...
  pds_presence_frequency %>%
    readr::write_csv(file.path(
      table_path,
      paste0(gene_filter, "_allele_frequency_mbe_pds_presence.csv")
    ))
  ...
  ggplot2::ggsave(filename = file.path(
    graphic_path,
    paste0(gene_filter, "_allele_frequency_mbe_pds_presence_donut.svg")),
  ...
  pds_presence_unclustered_frequency %>%
    readr::write_csv(file.path(
      table_path,
      paste0(gene_filter, "_allele_frequency_mbe_pds_presence_and_unclustered.csv")
    ))
  ...
  ggplot2::ggsave(filename = file.path(
    graphic_path,
    paste0(gene_filter, "_allele_frequency_mbe_pds_presence_and_unclustered_donut.svg")),
...
# Comparison of Full and Deduplicated Data Sets
  ...
  combined_presence_ranks %>%
    readr::write_csv(file.path(
      table_path,
      paste0(gene_filter, "_allele_frequency_mbe_mlst_pds_presence_ranks.csv")
    ))
}
```
...
# Comparison of Full and Deduplicated Data Sets
  ...
  dplyr::mutate(full_data_rank = dplyr::dense_rank(desc(isolates))) %>%
  dplyr::mutate(full_prop = isolates / .env$isolates %>% remove_unassigned_bla() %>% distinct(biosample) %>% nrow()) %>%
  {if(gene_filter == "OXA") dplyr::select(., allele, oxa_family, full_data_rank, full_prop) 
    else dplyr::select(., allele, full_data_rank, full_prop)} %>%
  dplyr::full_join(
    pds_deduplicated_frequency %>%
  ...
  mutate(
    abs_max_delta = pmax(abs(pds_dedupe_delta), abs(pds_unclust_delta), abs(pds_dedupe_unclust_delta), na.rm = TRUE) ,
    max_rank = pmin(full_data_rank, pds_deduplicated_rank, pds_unclustered_rank, pds_deduplicated_unclustered_rank, na.rm = TRUE),
    min_rank = pmax(full_data_rank, pds_deduplicated_rank, pds_unclustered_rank, pds_deduplicated_unclustered_rank, na.rm = TRUE),
    max_prop = pmax(full_prop, pds_dedupe_prop, pds_unclust_prop, pds_dedupe_unclust_prop, na.rm = TRUE),
  )
...
## Allele Presence and Absence - PDS Cluster - Isolates Browser
```{r, eval = FALSE}
for (i in 1:nrow(combinations)) {
  gene_filter <- combinations[[i, "name"]]
  gene_filter_label <- combinations[[i, "label"]]
  ...
  pds_presence_frequency %>%
    readr::write_csv(file.path(
      table_path,
      paste0(gene_filter, "_allele_frequency_isolates_pds_presence.csv")
    ))
  ...
  ggplot2::ggsave(filename = file.path(
    graphic_path,
    paste0(gene_filter, "_allele_frequency_isolates_pds_presence_donut.svg")),
    plot = pds_presence_frequency_plot
  ) 
  ...
  pds_presence_unclustered_frequency %>%
    readr::write_csv(file.path(
      table_path,
      paste0(gene_filter, "_allele_frequency_isolates_pds_presence_and_unclustered.csv")
    ))
  ...
  ggplot2::ggsave(filename = file.path(
    graphic_path,
    paste0(gene_filter, "_allele_frequency_isolates_pds_presence_and_unclustered_donut.svg")),
    plot = pds_presence_unclustered_frequency_plot
  )
# Comparison of Full and Deduplicated Data Sets
  ...
  mutate(
    abs_max_delta = pmax(abs(pds_presence_delta), abs(pds_unclust_delta), abs(pds_presence_unclust_delta), na.rm = TRUE) ,
    max_rank = pmin(full_data_rank, pds_presence_rank, pds_unclustered_rank, pds_presence_unclustered_rank, na.rm = TRUE),
    min_rank = pmax(full_data_rank, pds_presence_rank, pds_unclustered_rank, pds_presence_unclustered_rank, na.rm = TRUE),
    max_prop = pmax(full_prop, pds_presence_prop, pds_unclust_prop, pds_presence_unclust_prop, na.rm = TRUE),
  )
  
  combined_deduplicated_ranks %>%
    readr::write_csv(file.path(
      table_path,
      paste0(gene_filter, "_allele_frequency_isolates_presence_ranks.csv")
    ))
}
```
...
overview_bioproject_list_by_region <- isolates %>%
  ...
  overview_bioproject_distinct_count <- isolates %>%
...
for (i in 1:nrow(combinations)) {
  ...
  bioproject_presence_frequency %>%
    readr::write_csv(file.path(
      table_path,
      paste0(gene_filter, "_allele_frequency_isolates_bioproject_presence.csv")
    ))
  ...
  ggplot2::ggsave(filename = file.path(
    graphic_path,
    paste0(gene_filter, "_allele_frequency_isolates_bioproject_presence_donut.svg")),
