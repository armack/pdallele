...
## Settings, Filters, and Defaults

### Set output path and ensure directories exist
Enter
```{r, eval = FALSE}
...
depends greatly on the goals and scope of that analysis.

The following suggested defaults remove calls made using Hidden Markov Models
...
* `genes_of_interest` a character vector of genes to generate a full analysis for. Also used to
* `heatmap_palette` character vector of `viridis::scale_color_viridis()` color palettes corresponding to each element of `genes_of_interest` or a single value recycled for all genes
...
## Collection period groups
How many groups groups (of similar isolate counts) should collection periods be
split into?
```{r, eval = FALSE}
collection_period_groups <- 6L
```

## Unclear species string
...
### Prepare filtered Isolates Browser dataset

```{r, eval = FALSE}
isolates <- isolates_raw %>%
  clean_filter_alleles(filter = filter_alleles,
                       remove = remove_allele_types,
                       deduplicate = TRUE)

alleles_filtered <- isolates_raw %>%
  clean_filter_alleles(filter = filter_alleles,
                       deduplicate = TRUE)

```

### Prepare filtered MicroBIGG-E dataset

```{r, eval = FALSE}

```

## Isolates Browser Dataset
### Overview
#### Isolates and genes before and after filtering
```{r, eval = FALSE}
# Initial Isolate and Allele Counts
overview_unfiltered_isolate_count <- alleles_filtered %>%
  dplyr::distinct(biosample) %>%
  nrow()

overview_unfiltered_allele_count <- alleles_filtered %>%
  nrow()

overview_unfiltered_distinct_named_allele_count <-
  alleles_filtered %>%
  dplyr::distinct(protein, .keep_all = TRUE) %>%
  nrow()

overview_unfiltered_distinct_genes <- alleles_filtered %>%
  dplyr::distinct(gene) %>%
  dplyr::arrange(gene) %>%
  dplyr::pull() %>%
  paste(collapse = ", ")

overview_removed_alleles <- alleles_filtered %>%
  dplyr::filter(grepl(paste(remove_allele_types, collapse = "|"), allele_call)) %>%
  dplyr::count(allele_call)

overview_removed_alleles_text <-
  sprintf("%s (%i)",
          overview_removed_alleles$allele_call,
          overview_removed_alleles$n) %>%
  paste0(collapse = ", ")

# Cleaned Isolate and Allele Counts
...
isolates_overview_text <- c(
  "Unfiltered Isolates:" = overview_unfiltered_isolate_count,
  "Unfiltered Alleles:" = overview_unfiltered_allele_count,
  "Unfiltered Distinct Named Alleles:" = overview_unfiltered_distinct_named_allele_count,
  "Unfiltered Genes Present:" = paste0(overview_unfiltered_distinct_genes, "\n"),
  "Filtered Isolates:" = overview_isolate_count,
  "Filtered Alleles:" = overview_allele_count,
  "Filtered Distinct Named Alleles:" = overview_distinct_named_allele_count,
  "Filtered Genes Present:" = paste0(overview_distinct_genes, "\n"),
...
overview_isolate_counts_by_region %>%
  readr::write_csv(paste0(table_path, "overview_isolate_counts_by_region.csv"))
...
```{r, eval = FALSE}
# Load map data and add ICO3C codes
...
# Count of exclusive alleles by Species within Dataset
...
# Count of <gene_filter> alleles unique to a single species (in the dataset)
...
```{r, eval = FALSE}
mbe <- mbe_raw %>%
  filter_microbigge(coverage = min_coverage,
