...
## Step Three - Select a path to save downloaded data
```{r, eval = FALSE}
data_path <- "/Users/armack/Library/CloudStorage/OneDrive-CaseWesternReserveUniversity/Allele Distribution Data/Pseudomonas/2023-06-01/"
```
...
Set gcp_billing in _private_config.R or enter directly below if desired. See
bigrquery help at https://bigrquery.r-dbi.org for more information about
connecting to BigQuery using R.

```{r, eval = FALSE}
download_microbigge_bq(billing = gcp_billing,
                       path = data_path,
...
See `vignette("mlst")` for more information on downloading assemblies and determining MLST

Path to NCBI Assembly Summary. This is a very large (~500 MB) file so it may
be useful to keep one local copy when downloading data for multiple organisms
...
```{r, eval = FALSE}
genomes <-
  read_tsv(file = genome_data_path, skip = 2,
           col_types = "c-c----------------c---",
           col_names = c(
             "assembly",
             "bioproject",
...
```{r, eval = FALSE}
assemblies_to_download <- import_microbigge(file.path(data_path, "microbigge.tsv")) %>%
  distinct(assembly) %>%
  pull(assembly)
```
...
```{r, eval = FALSE}
genomes %>%
  filter(assembly %in% assemblies_to_download) %>%
  mutate(ftp_url = paste0(ftp_base, "/", basename(ftp_base), "_genomic.fna.gz")) %>%
  pull(ftp_url) %>%
  write_lines(file = file.path(data_path, "assemblies_to_download.txt"))
```
