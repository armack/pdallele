...
### Download and unzip assemblies This can be accomplished using any tool that
...
#### Reprocessing FastMLST Output Run the following command, where
`raw_mlst_path` is the path to the CSV output of FastMLST. This will
automatically save a reprocessed CSV file with "_reprocessed" appeneded in the
same directory. This reprocessed file should be used when loading data for
analysis using `pdallele`.
```{r, eval = FALSE}
