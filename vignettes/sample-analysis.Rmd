...
pds_combo_biosamples <- mbe %>%
  tidyr::drop_na(pds) %>%
  determine_combinations() %>%
  distinct(pds, combo, .keep_all = TRUE) %>%
  bind_rows(isolates %>% filter(is.na(pds)) %>% distinct(biosample, .keep_all = TRUE)) %>%
  pull(biosample)

mlst_combo_biosamples <- mbe %>%
  tidyr::drop_na(mlst) %>%
  determine_combinations() %>%
...
for (i in 1:nrow(combinations)) {
  ...
  mlst_deduplicated <- mbe %>%
    filter(biosample %in% mlst_combo_biosamples) %>%
    remove_unassigned_bla() %>%
    filter_attribute(allele, gene_filter) %>%
    tidyr::drop_na(mlst)
  ...
  pds_deduplicated <- mbe %>%
    filter(biosample %in% pds_combo_biosamples) %>%
    remove_unassigned_bla() %>%
    filter_attribute(allele, gene_filter) %>%
    tidyr::drop_na(pds)
  ...
  pds_deduplicated_unclustered <- mbe %>%
    filter(biosample %in% pds_combo_biosamples) %>%
    remove_unassigned_bla() %>%
    filter_attribute(allele, gene_filter)
...
for (i in 1:nrow(combinations)) {
  ...
  pds_deduplicated_unclustered <- isolates %>%
    filter(biosample %in% pds_combo_biosamples) %>%
    remove_unassigned_bla() %>%
    filter_attribute(allele, gene_filter)
