...
for (i in 1:nrow(combinations)) {
  ...
  gene_assigned_type_frequency <- isolates %>%
    ...
    count_isolates(allele, allele_markdown, total_rows) %>%
  ...
  gene_species_exclusive_allele_counts <- isolates %>%
    ...
    dplyr::group_by(species) %>%
    mutate(total_rows = nrow(.)) %>%
    count_isolates(allele, total_rows) %>%
    mutate(proportion = isolates / total_rows) %>%
    select(-total_rows)
  ...
  gene_multiples_overview_by_species <- isolates %>%
    ...
    dplyr::distinct(biosample, .keep_all = TRUE) %>%
    mutate(total_rows = nrow(.)) %>%
    count_isolates(species, total_rows) %>%
    mutate(proportion = isolates / total_rows) %>%
    select(-total_rows)
  ...
  gene_frequency_by_collection_period <- isolates %>%
    ...
    mutate(total_rows = dplyr::n()) %>%
    count_isolates(period, allele, allele_markdown, total_rows) %>%
    mutate(proportion = isolates / total_rows) %>%
    select(-total_rows) %>%
  ...
  gene_frequency_by_region <- isolates %>%
    ...
    mutate(total_rows = dplyr::n()) %>%
    count_isolates(region, allele, allele_markdown, total_rows) %>%
    mutate(proportion = isolates / total_rows) %>%
    select(-total_rows) %>%
  ...
  gene_frequency_by_species <- isolates %>%
    ...
    dplyr::group_by(species) %>%
    mutate(total_rows = dplyr::n()) %>%
    count_isolates(species, species_markdown, allele, allele_markdown, total_rows) %>%
    mutate(proportion = isolates / total_rows) %>%
    select(-total_rows) %>%
...
if("OXA" %in% combinations[["name"]]) {
  ...
  gene_assigned_type_frequency <- isolates %>%
    ...
    count_alleles(oxa_family, oxa_family_markdown)
  ...
  gene_frequency_by_collection_period <- isolates %>%
    ...
    remove_unassigned_bla() %>%
    tidyr::drop_na(period) %>%
    count_alleles(period, oxa_family, oxa_family_markdown) %>%
    relevel_numeric(period) %>%
  ...
  gene_frequency_by_collection_period_2000 %>%
    select(period, oxa_family, alleles) %>%
    readr::write_csv(file.path(
      table_path,
  ...
  gene_frequency_by_region <- isolates %>%
    ...
    remove_unassigned_bla() %>%
    tidyr::drop_na(region) %>%
    count_alleles(region, oxa_family, oxa_family_markdown)
...
for (gene_name in genes_of_interest) {
  ...
  mbe_goi_unassigned_allele_list <- mbe %>%
    ...
    tidyr::drop_na(ipg) %>%
    count_isolates(ipg_accession, oxa_family) %>%
...
## Alleles Deduplicated by MLST Sequence Type
```{r, eval = FALSE}
for (i in 1:nrow(combinations)) {
  gene_filter <- combinations[[i, "name"]]
  gene_filter_label <- combinations[[i, "label"]]
  ...
  mlst_deduplicated_frequency %>%
    readr::write_csv(file.path(
      table_path,
      paste0(gene_filter, "_allele_frequency_mlst_deduplicated.csv")
    ))
  ...
  ggplot2::ggsave(filename = file.path(
    graphic_path,
    paste0(gene_filter, "_allele_frequency_mlst_deduplicated_donut.svg")),
...
for (i in 1:nrow(combinations)) {
  ...
  pds_deduplicated_frequency <- pds_deduplicated %>%
    count_isolates(allele,
                  !!rlang::sym(paste0("allele", "_markdown"))) %>%
    mutate(porportion = isolates / nrow(pds_deduplicated))
  
  pds_deduplicated_frequency %>%
    readr::write_csv(file.path(
      table_path,
      paste0(gene_filter, "_allele_frequency_pds_deduplicated.csv")
    ))
  ...
  ggplot2::ggsave(filename = file.path(
    graphic_path,
    paste0(gene_filter, "_allele_frequency_pds_deduplicated_donut.svg")),
  ...
  pds_unclustered_frequency %>%
    readr::write_csv(file.path(
      table_path,
      paste0(gene_filter, "_allele_frequency_pds_unclustered.csv")
    ))
  ...
  ggplot2::ggsave(filename = file.path(
    graphic_path,
    paste0(gene_filter, "_allele_frequency_pds_unclustered_donut.svg")),
  ...
  pds_deduplicated_unclustered_frequency %>%
    readr::write_csv(file.path(
      table_path,
      paste0(gene_filter, "_allele_frequency_pds_deduplicated_and_unclustered.csv")
    ))
  ...
  ggplot2::ggsave(filename = file.path(
    graphic_path,
    paste0(gene_filter, "_allele_frequency_pds_deduplicated_and_unclustered_donut.svg")),
