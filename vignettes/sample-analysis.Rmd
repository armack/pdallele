...
for (i in 1:nrow(combinations)) {
  ...
  pds_presence_frequency <- pds_presence %>%
    ...
    mutate(proportion = isolates / length(unique(mlst_presence$pds))) %>%
    mutate(rank = dplyr::dense_rank(desc(isolates))) %>%
    left_join(isolates %>%
                filter_assigned_bla() %>%
                filter_attribute(allele, gene_filter) %>%
...
# Comparison of Full and Deduplicated Data Sets
  ...
  combined_deduplicated_ranks <- mbe %>%
    ...
    dplyr::full_join(
      mlst_presence_frequency %>%
        dplyr::select(allele, mlst_presence_rank = rank),
      by = "allele") %>%
    dplyr::full_join(
      pds_presence_frequency %>%
        dplyr::select(allele, pds_presence_rank = rank),
      by = "allele") %>%
    dplyr::full_join(
      pds_unclustered_frequency %>%
        dplyr::select(allele, pds_unclustered_rank = rank),
    ...
    dplyr::full_join(
      pds_presence_unclustered_frequency %>%
        dplyr::select(allele, pds_presence_unclustered_rank = rank),
      by = "allele") 
