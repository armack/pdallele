...
for (i in 1:nrow(combinations)) {
  ...
  gene_assigned_type_frequency <- isolates %>%
    ...
    count_isolates(!!gene_filter_type, !!rlang::sym(paste0(gene_filter_type_text, "_markdown")))
  ...
  gene_frequency_by_collection_period <- isolates %>%
    ...
    remove_unassigned_bla() %>%
    tidyr::drop_na(period) %>%
    count_isolates(period,!!gene_filter_type,!!gene_filter_type_markdown) %>%
  ...
  gene_frequency_by_collection_period_2000 <- isolates %>%
    ...
    remove_unassigned_bla() %>%
    tidyr::drop_na(period) %>%
    count_isolates(period,!!gene_filter_type,!!rlang::sym(paste0(gene_filter_type_text, "_markdown"))) %>%
    relevel_numeric(period) %>%
  ...
  gene_frequency_by_region <- isolates %>%
    ...
    remove_unassigned_bla() %>%
    tidyr::drop_na(region) %>%
    count_isolates(region,!!gene_filter_type,!!rlang::sym(paste0(gene_filter_type_text, "_markdown")))
...
for (i in 1:nrow(combinations)) {
  ...
  pds_deduplicated_frequency2 <- pds_deduplicated %>%
