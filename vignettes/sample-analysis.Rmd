...
carbapenemase_presence_over_time <- isolates %>%
  ...
  rename(carbapenemase_isolates = isolates) %>%
  left_join(isolates %>% count_isolates(period), by = "period") %>%
  rename(total_isolates = isolates) %>%
  mutate(proportion = carbapenemase_isolates / total_isolates) %>%
...
carbapenemase_presence_over_time_since_2000 <- isolates %>%
  ...
  rename(carbapenemase_isolates = isolates) %>%
  left_join(isolates %>% count_isolates(period_2000), by = "period_2000") %>%
  rename(total_isolates = isolates) %>%
  mutate(proportion = carbapenemase_isolates / total_isolates) %>%
...
for (i in 1:nrow(combinations)) {
  ...
  mlst_deduplicated <- mbe %>%
    ...
    filter_attribute(allele, gene_filter) %>%
    tidyr::drop_na(mlst) %>%
    dplyr::distinct(mlst, allele, .keep_all = TRUE)
  ...
  mlst_deduplicated_frequency <- mlst_deduplicated %>%
    ...
    mutate(porportion = isolates / nrow(mlst_deduplicated)) %>%
  mutate(rank = dplyr::dense_rank(desc(isolates)))
  ...
  pds_deduplicated_frequency <- pds_deduplicated %>%
    ...
    mutate(porportion = isolates / nrow(pds_deduplicated)) %>%
  mutate(rank = dplyr::dense_rank(desc(isolates)))
  ...
  pds_unclustered_frequency <- pds_unclustered %>%
    ...
    mutate(porportion = isolates / nrow(pds_unclustered)) %>%
  mutate(rank = dplyr::dense_rank(desc(isolates)))
  ...
  pds_deduplicated_unclustered_frequency <- pds_deduplicated_unclustered %>%
    ...
    mutate(porportion = isolates / nrow(pds_deduplicated_unclustered)) %>%
  mutate(rank = dplyr::dense_rank(desc(isolates)))
...
# Comparison of Full and Deduplicated Data Sets
  ...
  combined_deduplicated_ranks %>%
    readr::write_csv(file.path(
      table_path,
      paste0(gene_filter, "_allele_frequency_mbe_deduplicated_ranks.csv")
    ))
}
```
## Deduplicated Alleles - MLST & PDS Cluster - Isolates Browser
```{r, eval = FALSE}
for (i in 1:nrow(combinations)) {
  gene_filter <- combinations[[i, "name"]]
  gene_filter_label <- combinations[[i, "label"]]
...
## Stats by BioProject & BioProject Center
```{r, eval = FALSE}
## Counts by Region
...
overview_bioproject_center_list_by_region <- isolates %>%
  distinct(biosample, .keep_all = TRUE) %>%
  count_isolates(region, bioproject_center, sort = FALSE) %>%
  group_by(region) %>%
  mutate(proportion = isolates/sum(isolates)) %>%
  arrange(region, desc(isolates))

  overview_bioproject_list_by_region %>%
    readr::write_csv(file.path(table_path, "bioproject_center_list_by_region.csv"))
  ...
  left_join(
    overview_bioproject_center_list_by_region %>%
      summarize(
        n_bpc = n(),
        max_bpc_isolates = max(isolates),
        median_bpc_isolates = median(isolates),
        mean_bpc_isolates = round(mean(isolates), digits = 1)
      ), by = "region"
  )
...
## Deduplicated Alleles - BioProject & BioProject Center
```{r, eval = FALSE}
for (i in 1:nrow(combinations)) {
  gene_filter <- combinations[[i, "name"]]
  gene_filter_label <- combinations[[i, "label"]]
  ...
  bioproject_deduplicated <- isolates %>%
    ...
    dplyr::distinct(bioproject, allele, .keep_all = TRUE)
  ...
  ggplot2::ggsave(filename = file.path(
    ...
    # BioProject Center Deduplicated - Assigned allele frequency
  bioproject_center_deduplicated <- isolates %>%
    remove_unassigned_bla() %>%
    filter_attribute(allele, gene_filter) %>%
    tidyr::drop_na(bioproject_center) %>%
    dplyr::distinct(bioproject_center, allele, .keep_all = TRUE)
  
  bioproject_center_deduplicated_frequency <- bioproject_center_deduplicated %>%
    {if(gene_filter == "OXA") count_isolates(., allele, allele_markdown, oxa_family) 
      else count_isolates(., allele, allele_markdown)} %>%
    mutate(porportion = isolates / nrow(bioproject_center_deduplicated)) %>%
  mutate(rank = dplyr::dense_rank(desc(isolates)))
  
  bioproject_deduplicated_frequency %>%
    readr::write_csv(file.path(
      table_path,
      paste0(gene_filter, "_allele_frequency_isolates_bioproject_center_deduplicated.csv")
    ))
  
  bioproject_center_deduplicated_frequency_plot <-
    bioproject_center_deduplicated_frequency %>%
    top_n_across_groups(allele,
                        allele_markdown,
                        count = isolates,
                        n = 10) %>%
  ...
  ggplot2::ggsave(filename = file.path(
    graphic_path,
    paste0(gene_filter, "_allele_frequency_isolates_bioproject_center_deduplicated_donut.svg")),
    plot = bioproject_center_deduplicated_frequency_plot
  ) 
    
# Comparison of Full and Deduplicated Data Sets

  combined_deduplicated_bioproject_ranks <- isolates %>%
    remove_unassigned_bla() %>%
    filter_attribute(allele, gene_filter) %>%
    {if(gene_filter == "OXA") count_isolates(., allele, oxa_family) 
      else count_isolates(., allele)} %>%
    ...
    dplyr::full_join(
      bioproject_deduplicated_frequency %>%
        dplyr::select(allele, bioproject_deduplicated_rank = rank),
      by = "allele") %>%
    dplyr::full_join(
      bioproject_center_deduplicated_frequency %>%
        dplyr::select(allele, bioproject_center_deduplicated_rank = rank),
      by = "allele") 
  
  combined_deduplicated_bioproject_ranks %>%
    readr::write_csv(file.path(
      table_path,
      paste0(gene_filter, "_allele_frequency_isolates_deduplicated_bioproject_ranks.csv")
    ))
}
```
